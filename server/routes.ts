...
import { generateChart, type Timeframe } from "./chartGenerator";
import { createChartJob, getChartJob, processChartAnalysisJob, type ChartAnalysisResult } from "./chartAnalysis";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Generate a chart from a ticker symbol
  app.post('/api/chart/generate', async (req, res) => {
    try {
      ...
      const validTimeframes: Timeframe[] = ['1D', '5D', '1M', '3M', '6M', '1Y'];
      const tf = timeframe || '3M';
      if (!validTimeframes.includes(tf)) {
        return res.status(400).json({ success: false, message: 'Invalid timeframe' });
      }

      console.log(`[Chart] Generating chart for ${symbol} with timeframe ${tf}`);
      
      const { buffer, chartData } = await generateChart(symbol.toUpperCase(), tf);
      ...
      return res.json({
        ...
        indicators: {
          lastClose: chartData.candles[chartData.candles.length - 1]?.close,
          rsi: chartData.indicators.rsi[chartData.indicators.rsi.length - 1],
          ema9: chartData.indicators.ema9[chartData.indicators.ema9.length - 1],
          ema21: chartData.indicators.ema21[chartData.indicators.ema21.length - 1],
        },
      });
    } catch (error) {
      ...
      return res.status(500).json({
        success: false,
        message: error instanceof Error ? error.message : 'Failed to generate chart',
      });
    }
  });

  // Start an async chart analysis job
  app.post('/api/chart/analyze', upload.single('chart'), async (req, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ success: false, message: 'Authentication required' });
      }
      ...
      // Check if image was uploaded as file or base64 in body
      if (req.file) {
        ...
        return res.status(400).json({ success: false, message: 'Chart image is required' });
      }

      console.log(`[Chart Analysis] Starting analysis for ${symbol || 'uploaded chart'}`);

      // Create job and start processing asynchronously
      const job = createChartJob(req.user.id);
      
      // Process in background (fire-and-forget)
      processChartAnalysisJob(job.id, imageBase64, symbol);

      return res.json({
        success: true,
        jobId: job.id,
        status: 'queued',
        message: 'Analysis job created. Poll /api/chart/job/:jobId for results.',
      });
    } catch (error) {
      console.error('Chart analysis error:', error);
      return res.status(500).json({
        success: false,
        message: error instanceof Error ? error.message : 'Failed to start chart analysis',
      });
    }
  });

  // Get chart analysis job status
  app.get('/api/chart/job/:jobId', async (req, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ success: false, message: 'Authentication required' });
      }
      ...
      const { jobId } = req.params;
      const job = getChartJob(jobId, req.user.id);
      ...
      console.error('Chart job status error:', error);
      return res.status(500).json({
        success: false,
        message: error instanceof Error ? error.message : 'Failed to check job status',
      });
